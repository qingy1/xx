  ...
  <div class="endless-tower">
    ...
    <div class="battle-info">
      ...
      <div class="monster-info" v-if="currentMonster">
        ...
        <div class="health-bar">
          <div class="health-fill" :style="{ width: `${(currentMonster.health / currentMonster.maxHealth) * 100}%` }">
          </div>
          <span class="health-text">{{ Math.max(0, currentMonster.health) }} / {{ currentMonster.maxHealth }}</span>
        </div>
    ...
    <div class="actions">
      <el-button @click="toggleAutoFight" :type="isAutoFighting ? 'danger' : 'primary'" :disabled="player.health <= 0">
...
export default {
  ...
  mounted() {
    this.player = JSON.parse(JSON.stringify(this.$store.player)); // 深拷贝玩家数据
    this.highestFloor = this.player.highestTowerFloor || 1; // 获取最高层数
    this.currentFloor = 1
    if (this.highestFloor > 1) this.currentFloor = this.highestFloor - 1; // 设置当前层数为最高层数
